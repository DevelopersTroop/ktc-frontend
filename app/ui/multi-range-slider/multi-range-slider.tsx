...
const MultiRangeSlider = ({min, max, currentLow, currentHigh, onChange}: {
    min: number;
    max: number;
    currentLow: number;
    currentHigh: number;
    onChange: ({min, max, currentLow,currentHigh }: { min: number; max: number; currentLow:number; currentHigh:number }) => void;
}) => {
    ...
    return (
        <>
            ...
            <div className={"inline-block mt-5 font-medium"}>
                Price: ${minVal} - ${maxVal}
            </div>
        </>
    );

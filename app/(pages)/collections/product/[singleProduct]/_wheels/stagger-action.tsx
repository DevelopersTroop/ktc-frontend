...
export const StaggeredActionButton: React.FC<{ product: TInventoryItem }> = ({
  product,
}) => {
  ...
  const addProductToCart = async (
    quantity: number,
    meta?: RootState["persisted"]["cart"]["products"][string]["metaData"]
  ) => {
    ...
    <div className="flex flex-col gap-y-4">
      <div>
        <div className="flex items-center justify-center gap-2">
          ...
          <h2 className="uppercase font-bold text-gray-700 text-lg">
            Square Setup
          </h2>
          <hr className="w-6 border-gray-700 border-[1.5px]" />
        </div>
        <Button
          onClick={handleSquareSetup}
          className="w-full uppercase font-medium h-12 text-lg bg-primary hover:bg-primary-hover"
        >
          buy all (4) of this size
        </Button>
      </div>
      <div>
        <div className="flex items-center justify-center gap-2">

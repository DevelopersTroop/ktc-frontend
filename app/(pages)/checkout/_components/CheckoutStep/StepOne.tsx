...
export const StepOne: React.FC<ICheckoutStepProps> = ({ setStep }) => {
  const {
    isValidZipCode,
    validatedZipCode,
    setZipCodeAddress,
  ...
  const handleValidate = async () => {
    setLoading(true);
    toast.loading("Validating ZIP code...");

    try {
      // First API call for ZIP validation
      const response = await fetch(`https://api.zippopotam.us/us/${code}`);
      const data = await response.json();

      if (data && data.places.length) {
        // Using direct values instead of state
        const address = data.places[0];
        const newLocation = {
          lat: address.latitude,
          lng: address.longitude,
        };
        // Second API call for address details
        ...
        // Update address and show success
        if (addressData?.display_name) {
          setZipCodeAddress(addressData.display_name);
        }

        setValidatedZipCode(data?.["post code"] || validatedZipCode);
        toast.success("ZIP code validated successfully");
      } else {
        // Handle invalid ZIP
      ...
      // Handle errors
      toast.error("An error occurred while validating the ZIP code.");
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="flex flex-col gap-y-6">
      <div className="py-smx px-md gap-lg grid grid-cols-1 rounded-xs bg-[#F7F7F7] lg:grid-cols-2">
        <div className="flex flex-col gap-y-3">
          <div className="pt-3">
            <p className="leading-snug">
              We can ship your tires to an independent Recommended Installers
              who can professionally install the for a reasonable cost.
            </p>
            <Link
              href={"/"}
              className="mb-6 mt-3 block font-semibold underline"
            >
              Learn more about installation pricing and our Price Pledge
              Program.
            </Link>
          </div>
          <div className="space-y-3">
            <Label>Change ZIP code to update installer options</Label>
            <div className="gap-xs flex max-w-lg">
              <Input
                value={code || validatedZipCode}
        ...
        <div className="p-smx space-y-2 rounded-xl border border-border bg-white">
      ...
      <div className="grid grid-cols-10 gap-8">
        ...
        <div className="z-10 col-span-10 h-[60vh] overflow-hidden rounded-xl border border-border lg:col-span-6">
          <div className="py-xs px-smx flex items-center justify-between">
            <div className="flex gap-[1.5rem]">
              ...
              <div className="py-xs px-smx flex items-center gap-4 font-semibold">
                <svg
                  width="28"
                  height="28"
                  viewBox="0 0 28 28"
            ...
            <button
              ...
              className="h-fit bg-none font-semibold underline"
            >
              Request a Dealer
            </button>
          </div>

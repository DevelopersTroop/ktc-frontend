...
import { useLazyGetProductListQuery } from "@/app/globalRedux/api/product";
...
const SearchSuggestion = ({ setOpen, searchInput }: { setOpen: React.Dispatch<React.SetStateAction<boolean>>, searchInput: string }) => {
    const router = useRouter();
    // const { getProducts } = useProductFetch(); // Using the custom hook to fetch products
    const { toggleFilterValue } = useFilterSync()

    const [trigger, { isLoading, isFetching, data }] = useLazyGetProductListQuery();


    useEffect(() => {
        const handler = setTimeout(() => {
            trigger(toggleFilterValue("q", searchInput, false));
        }, 300);
        ...
        <>
            {
                (isLoading || isFetching) ? (
                    ...
                    data?.products ?
                        <>
                            <div className='flex flex-col bg-white w-full overflow-y-auto' style={{ maxHeight: 'calc(100dvh - 80px)' }}>
                                {
                                    data?.products?.map((product) => {
                                        if (product?.category?.title === "Accessories" || product?.category?.title === "Tire") {
                                        ...
                                        return (
                                            <button type="button" key={product._id} onClick={handleClick} className='block border-b py-1 border-gray-300 bg-white hover:bg-gray-100 text-left'>
                                                <div className='flex items-start gap-4 px-10 '>
                                                    <div className="w-[60px]">
                                                        <Image className='rounded w-full' src={imageURL ?? ""} alt={product?.title ?? ""} width={60} height={60} />
                                                    </div>
                                                    <div className='flex w-full flex-col gap-1 justify-start'>
                                                        ...
                                                        {/* Price */}
                                                        <p>Price: {(product.msrp * 4).toFixed(2)}</p>
                                                    </div>
                                                </div>
                                            </button>)
                                    })
